from typing import Optional, Sequence, Tuple, Union, Type

from m5.objects import (
    AddrRange,
    DRAMInterface,
    InspectedMemory,
    Port,
)

from gem5.components.cachehierarchies.classic.caches.l1dcache import L1DCache

from gem5.components.boards.abstract_board import AbstractBoard
from gem5.components.memory.memory import ChanneledMemory
from gem5.utils.override import overrides
from m5.objects import DDR4_2400_8x8


class InspectedChanneledMemory(ChanneledMemory):
    def __init__(
        self,
        dram_interface_class: Type[DRAMInterface],
        num_channels: Union[int, str],
        interleaving_size: Union[int, str],
        size: Optional[str] = None,
        addr_mapping: Optional[str] = None,
        inspection_buffer_entries: int = 8,
        ## Add insp_window, num_insp_units, insp_tot_latency, here.
        insp_tot_latency: int = 1,
        insp_window: int = 1,
        num_insp_units: int = 1,
        output_buffer_entries: int = 8,
        response_buffer_entries: int = 32,
    ) -> None:
        super().__init__(
            dram_interface_class,
            num_channels,
            interleaving_size,
            size=size,
            addr_mapping=addr_mapping,
        )
        self.inspectors = [
            InspectedMemory(
                inspection_buffer_entries=inspection_buffer_entries,
                ## Add insp_window, num_insp_units, insp_tot_latency, here.
                insp_tot_latency = insp_tot_latency,
                insp_window = insp_window,
                num_insp_units = num_insp_units,
                output_buffer_entries=output_buffer_entries,
                response_buffer_entries=response_buffer_entries,
            )
            for _ in range(num_channels)
        ]

        self.metadata_cache = L1DCache("32KiB")

    @overrides(ChanneledMemory)
    def incorporate_memory(self, board: AbstractBoard) -> None:
        super().incorporate_memory(board)
        for inspector in self.inspectors:
            inspector.metadata_cache_request_port = self.metadata_cache.cpu_side
        for ctrl in self.mem_ctrl:
            self.metadata_cache.mem_side = ctrl.port

    @overrides(ChanneledMemory)
    def get_mem_ports(self) -> Sequence[Tuple[AddrRange, Port]]:
        return [
            (ctrl.dram.range, inspector.cpu_side_port)
            for ctrl, inspector in zip(self.mem_ctrl, self.inspectors)
        ]


class HW5DDR4(InspectedChanneledMemory):
    def __init__(self):
        super().__init__(
            dram_interface_class=DDR4_2400_8x8,
            num_channels=1,
            size="2 GiB",
            interleaving_size=128,
        )
